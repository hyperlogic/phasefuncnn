    ...
    for frame in trange(num_frames):
        ...
        for i in range(s.num_joints):
            ...
            if s.has_pos(joint_name):
                ...
                if s.is_root(joint_name):
                    root_motion += glm.vec3(x_pos, y_pos, z_pos)
                    x_pos = root_motion[0]
                    y_pos = root_motion[1]
                    z_pos = root_motion[2]
            else:
            ...
            xrot = glm.mat4(glm.normalize(glm.quat(x_rot, glm.vec3(1, 0, 0))))
            yrot = glm.mat4(glm.normalize(glm.quat(y_rot, glm.vec3(0, 1, 0))))
            zrot = glm.mat4(glm.normalize(glm.quat(z_rot, glm.vec3(0, 0, 1))))
            m = zrot * xrot * yrot
